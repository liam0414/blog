...
> Otsu's method: automatically determine the threshold value
...
> IsoData method: iteratively determine the threshold value
...
- Power-law (gamma) transformation: $s = c r^{\gamma}$, they are good for enhancing the contrast of images
- Both are used to expand the range of dark intensities
...
Assume the ture noise free image is $g(x, y)$ and the noisy image is $f_i(x, y) = g_i(x, y) + n_i(x, y)$ for i = 1...n, where n(x, y) is the zero mean independent and identically distributed noise. We can write the average of the noisy images as:
...
**Question: What is the variance of the average image?**
...
You can find all the code examples I played with in this repo https://github.com/ligam0414/COMP9517
